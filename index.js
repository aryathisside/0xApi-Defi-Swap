...
//    tokens = tokenListJSON.tokens[3593]
//    tokens1 = tokenListJSON.tokens[793]
//   //  Object.assign( ,tokens);
//   console.log("token",tokens1)
...
//   let response = await fetch('https://tokens.coingecko.com/uniswap/all.json');
//   let tokenListJSON = await response.json();
//   let tokens = tokenListJSON.tokens[793]
//   let tokens1 = tokenListJSON.tokens[3593]
...
//   let parent1 = document.getElementById("token_list1");
...
//     div1.className = "token_row1";
//     // For each row, display the token image and symbol
//     let html = `
//     <img class="token_list_img" src="${tokens.logoURI}">
//       <span class="token_list_text">${tokens.symbol}</span>
//       `;
//     div.innerHTML = html;
//     div.onclick = () => {
//       selectToken(tokens);
...
//     let html1 = `
//     <img class="token_list_img" src="${tokens1.logoURI}">
//       <span class="token_list_text">${tokens1.symbol}</span>
//       `;
...
async function listAvailableTokens(){
  console.log("initializing");
  let response = await fetch('https://raw.githubusercontent.com/aryathisside/token-list/main/list.json');
  let tokenListJSON = await response.json();
  ...
  // tknDAI = tokenListJSON.tokens[0]
  // tknWETH = tokenListJSON.tokens[1]
  //   console.log("tokens:", tknDAI);
  //   console.log("tokens:", tknWETH);
  ...
  for (const i in tokenListJSON){
    ...
    let html = `
    <img class="token_list_img" src="${tokenListJSON[i].logoURI}">
      <span class="token_list_text">${tokenListJSON[i].symbol}</span>
      `;
    div.innerHTML = html;
    div.onclick = () => {
      selectToken(tokenListJSON[i]);
    };
    parent.appendChild(div);``
  }
}
...
// console.log("akdk") 
  ...
  // const fromTokenAddress = web3.eth.currentProvider;
